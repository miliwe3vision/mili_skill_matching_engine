...
# -----------------------------
# ACTIVE TASK ESTIMATION (will be replaced by Supabase fetch)
# -----------------------------
def estimate_active_tasks_from_supabase(emp_id):
    try:
        response = supabase.from_("employee_workload").select("active_tasks").eq("emp_id", emp_id).execute()
        if response.data and len(response.data) > 0:
            return response.data[0]["active_tasks"]
        else:
            print(f"No active tasks found for employee ID: {emp_id}. Returning default 0.")
            return 0
    except Exception as e:
        print(f"Error fetching active tasks from Supabase: {e}. Returning default 0.")
        return 0
